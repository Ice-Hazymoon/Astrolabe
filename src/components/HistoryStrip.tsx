...
export function HistoryStrip({ onClose }: HistoryStripProps = {}) {
  ...
  return (
    <section className="relative z-10 shrink-0 px-3 sm:px-4 pb-3 sm:pb-4 pt-1">
      <div className="surface relative rounded-[var(--radius-lg)] flex items-stretch h-[100px] sm:h-[110px] overflow-hidden">
        <div className="flex flex-col items-start justify-between gap-1 shrink-0 pl-4 pr-3 py-3 border-r border-[color:var(--color-line-soft)]/60">
          ...
          <span className="text-mono text-[11px] text-[color:var(--color-text)] tabular-nums">
        ...
        <div className="relative flex-1 min-w-0">
          {entries.length === 0 ? (
            <div className="absolute inset-0 grid place-items-center text-[12px] text-[color:var(--color-text-muted)] tracking-wide">
              <span>{t('empty')}</span>
            </div>
          ) : (
            <ul className="absolute inset-0 flex items-center gap-2.5 overflow-x-auto px-3">
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className={cn(
                          'relative h-[78px] sm:h-[86px] aspect-[4/3] rounded-[var(--radius-sm)] overflow-hidden block',
